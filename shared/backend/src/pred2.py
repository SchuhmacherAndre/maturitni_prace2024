...
results = model.predict(source=im1, save=True, conf=0.35)
import math
...
# if any value from array2 is inside array1, pass
#if any value from array2 ISNT in array1 add it to list
...
last_three_throws_cursor = col.find({}, {"i_p": 1}, sort=[('_id', pymongo.ASCENDING)]).limit(3)
last_three_throws = [doc["i_p"] for doc in last_three_throws_cursor]

spe = col.find({}, {"s_p": 1}, sort=[('_id', pymongo.ASCENDING)]).limit(3)
sbe = [doc["s_p"] for doc in spe]

print(sbe)

arr = []
arr2 = []
if last_three_throws:
    for i in last_three_throws:
        for x in range(3):
            arr.append(i[-(x + 1)])
...
def compare_arrays(array1, array2):
    ...
    for val2 in array2:
        if val2 not in array1:
            unmatched_values.append(val2)

    return unmatched_values



# Get the current value of t_p from the document
current_t_p = document.get("t_p", 0)  # Default value is 0 if t_p doesn't exist


    
unm = compare_arrays(arr, pointsByCount)
unm2 = compare_arrays(arr2, pointsByRing)
x = 0
for i in unm:
    x += i
new_t_p = current_t_p - x

# Update the document with the new value of t_p
update_query = {
    "$push": {
        "s_p": {"$each": unm2},
        "i_p": {"$each": pointsByCount}
    },
    "$set": {
        "t_p": new_t_p
    }
